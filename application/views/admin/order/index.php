		...
		<table cellpadding="0" cellspacing="0" width="100%" class="sTable mTable myTable" id="checkAll">
            ...
            <?php foreach($list as $row):?>
			    <tr class="row_<?php echo $row->id?>">
					...
					<td class="textC"> <?php echo$row->product_name?></td>
					<td class="textC"> <?php echo$row->product_price?></td>
					<td class="textC"><?php echo $row->qty?></td>
					<td class="textC"><?php
					   if($row->tt_status==0) echo 'Chưa thanh toán';
					   elseif($row->tt_status==1)echo 'Đã thanh toán';
					   else echo 'Giao dịch đã hủy';
					?></td>
					<td class="textC"><?php
					   if($row->status==0) echo 'Chưa giao hàng';
					   elseif($row->status==1)echo 'Đã giao';
					   else echo 'Đơn hàng đã hủy';
					?></td>
					<td class="textC"><?php echo $row->created?></td>
					
					<td class="option textC">
						<a href="<?php echo admin_url('order/view/'.$row->id)?>" target="_blank" class="tipS" title="Xem chi tiết giao dịch">
